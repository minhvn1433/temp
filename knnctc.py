...
class KNNWrapper(object):
    def __init__(self, dstore_size, dstore_dir, dimension,
            ...
            no_load_keys=False, move_dstore_to_mem=False, knn_gpu=None,
            recompute_dists = False,
            k=1024, lmbda=0.25, knn_temp=1.0, probe=32,
            query_time_pruning=False):
        ...
        self.query_time_pruning = query_time_pruning
    ...
    def break_into(self, model):
        self.model = model
        self.broken_into = True
        self.reconstruct_index, self.index = self.setup_faiss()
        self.is_encoder_decoder = model.config.is_encoder_decoder
        self.pad_token_id = getattr(model.config, "pad_token_id", None)
    ...
    def pre_forward_hook(self, input_values=None, attention_mask=None, labels=None, **kwargs):
        self.labels = labels
        return self.original_forward_func(input_values=input_values, labels=labels, attention_mask=attention_mask, **kwargs)

    def post_forward_hook(self, module, input, output):
        batch, time_dim, vocab_size = output.shape
        shift = 0 # hard coded value for wav2vec2 encoder
        lm_logits = output
        ...
        nonpad_mask = torch.ones([batch, time_dim], dtype=torch.bool).to(self.device)
        ...
        if self.query_time_pruning:
            top_indices = lm_logits.argmax(dim=-1)
            search_mask = (top_indices != self.pad_token_id)
            active_queries = queries[search_mask]
            active_lm_logits = lm_logits[search_mask]
        else:
            search_mask = None
            active_queries = queries
            active_lm_logits = lm_logits
        
        dists, knns = self.get_knns(active_queries) # (nonpad batch * time, k)
        if self.recompute_dists:
            knns_vecs = torch.from_numpy(self.keys[knns]).to(self.device)
            dists = self.dist_func(active_queries, knns_vecs) 
        
        neg_dists = -dists
        knn_log_probs, _ = self.knns_to_log_prob(knns, neg_dists)
        
        interpolated_scores = KNNWrapper.interpolate(knn_log_probs, active_lm_logits, self.lmbda) # (nonpad, vocab)
        if self.query_time_pruning:
            lm_logits[search_mask] = interpolated_scores
        else:
            lm_logits = interpolated_scores
        output[nonpad_mask] = lm_logits
        return output
    ...
    def break_out(self):
        for h in self.hook_handles:
            h.remove()
        if self.model is not None and self.broken_into is not None:
    ...
    def get_model_embedding_layer(model_type):
        if model_type.startswith("wav2vec2"):
            return lambda model: model.base_model.feature_projection
    ...
    model_layer_to_capture = {
        "wav2vec2": {
            KEY_TYPE.last_ffn_input: (lambda model: model.base_model.encoder.layers[-1].layer_norm, False),
            KEY_TYPE.last_ffn_output: (lambda model: model.base_model.encoder.layers[-1].final_layer_norm, False),
        },
    }


class KNNSaver(object):
    def __init__(self, dstore_size, dstore_dir, dimension, knn_keytype=None, dstore_pruning=False, use_pseudo_labels=True):
        self.dstore_size = dstore_size
        self.dstore_dir = dstore_dir
        self.dimension = dimension
        self.knn_keytype = KEY_TYPE.last_ffn_input if knn_keytype is None else knn_keytype
        ...
        self.dstore_pruning = dstore_pruning
    ...
    def post_forward_hook(self, module, input, output):
        ...
        if self.use_pseudo_labels:
            captured_values = torch.argmax(logits, dim=-1)
        else:
            captured_values = self.label
        ...
        assert captured_keys.shape[0] == captured_values.shape[0], \
            f"Length Mismatch: Keys {captured_keys.shape[0]} vs Values {captured_values.shape[0]}. " \
            "Real labels must be aligned to audio frames (T) to use them here."

        nonpad_mask = (captured_values != -100)
        if self.dstore_pruning:
            nonpad_mask &= (captured_values != self.pad_token_id)
