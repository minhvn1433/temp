...
from knnctc import KNNWrapper, KNNSaver, KEY_TYPE, DIST
...
class DataTrainingArguments:
    ...
    bos_token: str = field(
        default="<s>",
        metadata={"help": "The beginning of sentence for the tokenizer"},
    )
    eos_token: str = field(
        default="</s>",
        metadata={"help": "The end of sentence token for the tokenizer"},
    )
...
class KNNArguments:
    """
    Arguments pertaining to the K-Nearest Neighbors configuration and retrieval settings for the model.
    """
    ...
    dstore_dir: str = field(default="datastore")
    ...
    dstore_pruning: bool = field(default=False)
    query_time_pruning: bool = field(default=False)
    use_pseudo_labels: bool = field(default=False)
...
def create_vocabulary_from_data(
    ...
    #     vocabs.values(),
    # )
    all_vocab_sets = [set(dataset["vocab"][0]) for dataset in vocabs.values()]
    vocab_set = set().union(*all_vocab_sets)
...
def main():
    ...
    if training_args.do_train or knn_args.build_index:
    ...
    trainer = Trainer(
        ...
        train_dataset=vectorized_datasets["train"] if (training_args.do_train or knn_args.build_index) else None,
    ...
    if knn_args.build_index:
        knn_wrapper = KNNSaver(
            ...
            dstore_pruning = knn_args.dstore_pruning,
            use_pseudo_labels = knn_args.use_pseudo_labels,
        )
    elif knn_args.knn:
    ...
    if knn_args.build_index:
        ...
        trainer.predict(vectorized_datasets["train"])
        knn_wrapper.build_index()

        dstore_index = knn_wrapper.show_dstore_index()
        print("the total datastore size is :" + str(dstore_index))
